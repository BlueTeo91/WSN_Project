...
void setup()
{
  ...
  if(!SD.exists("test.txt")){
    ...
    if(!SD.exists("test.txt")) 
      Serial.println("ERROR creation test.txt");  
    else
      Serial.println("test.txt SUCCESSFULL created");
  }
  else
    Serial.println("test.txt already exist!");
...
void loop()
{
 /* // make a string for assembling the data to log:
  ...
  if (dataFile) {
    ...
    Serial.println("datalog2.txt exists.");
  }
  else {
    Serial.println("datalog2.txt doesn't exist.");  
  }

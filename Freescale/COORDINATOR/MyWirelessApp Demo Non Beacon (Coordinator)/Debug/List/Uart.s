...
//                    (Coordinator)\PLM\Source\UART\Uart.c                    /
//    Command line =  "C:\Documents and Settings\Administrator\My             /
//                    Documents\MyWirelessApp Demo Non Beacon                 /
//                    (Coordinator)\MyWirelessApp Demo Non Beacon             /
//                    (Coordinator)\PLM\Source\UART\Uart.c" -D                /
...
// C:\Documents and Settings\Administrator\My Documents\MyWirelessApp Demo Non Beacon (Coordinator)\MyWirelessApp Demo Non Beacon (Coordinator)\PLM\Source\UART\Uart.c
//    1 /*****************************************************************************
...
//  155   if (uart1_ungetFlag) {
        ...
        LDRB     R0,[R0, #+0]
        CMP      R0,#+0
        BEQ      ??Uart1_GetByteFromRxBuffer_0
...
//  191   return ((Uart1_TxInProgress) || (UART1_REGS_P->Utxcon != 32));
        ...
        LDRB     R0,[R0, #+0]
        CMP      R0,#+0
        BNE      ??Uart1_IsTxActive_0
        LDR      R0,??DataTable8  ;; 0x80005010
        LDR      R0,[R0, #+0]
        CMP      R0,#+32
        BEQ      ??Uart1_IsTxActive_1
??Uart1_IsTxActive_0:
        MOVS     R0,#+1
        B        ??Uart1_IsTxActive_2
??Uart1_IsTxActive_1:
        MOVS     R0,#+0
??Uart1_IsTxActive_2:
...
//  344   UartOpenCloseTransceiver(UART_1, BIT_RX_EN, NULL != pfSci1RxCallBack);  
        ...
        LDR      R0,[R0, #+0]
        CMP      R0,#+0
        BEQ      ??Uart1_SetRxCallBack_0
        MOVS     R2,#+1
        B        ??Uart1_SetRxCallBack_1
...
GpioUart1Init:
        LDR      R1,??DataTable12_13  ;; 0x80000030
        LDR      R1,[R1, #+0]
        MOVS     R2,#+160
...
//  797   GPIO.PuEnLo  |= (GPIO_UART1_RTS_bit | GPIO_UART1_RX_bit);  // Pull-up enable
        LDR      R1,??DataTable12_14  ;; 0x80000010
        LDR      R1,[R1, #+0]
        MOVS     R2,#+160
        LSLS     R2,R2,#+10       ;; #+163840
        ORRS     R2,R2,R1
...
//  800   tmpReg = GPIO.FuncSel0 & ~((FN_MASK << GPIO_UART1_RX_fnpos) | (FN_MASK << GPIO_UART1_TX_fnpos));
        LDR      R1,??DataTable12_15  ;; 0x80000018
        LDR      R1,[R1, #+0]
...
//  803   GPIO.FuncSel1 = tmpReg | ((FN_ALT << GPIO_UART1_CTS_fnpos) | (FN_ALT << GPIO_UART1_RTS_fnpos));
        MOVS     R1,#+5
        ORRS     R1,R1,R0
        LDR      R2,??DataTable12_16  ;; 0x8000001c
        STR      R1,[R2, #+0]
//  804   #else
...
//  827   | (FN_MASK << GPIO_UART2_RX_fnpos) | (FN_MASK << GPIO_UART2_TX_fnpos));
        LDR      R1,??DataTable12_16  ;; 0x8000001c
        LDR      R1,[R1, #+0]
        LDR      R2,??DataTable12_17  ;; 0xfffff00f
        ANDS     R2,R2,R1
        MOVS     R0,R2
...
??DataTable12_3:
        ...
        DATA
??DataTable12_4:
        DC32     maSci1TxCallbackLeadingIndex

        SECTION `.text`:CODE:NOROOT(2)
        DATA
??DataTable12_5:
        DC32     maSci1TxCallbackTable
...
??DataTable12_9:
        ...
        DATA
??DataTable12_10:
        DC32     maSci1TxCallbackTrailingIndex

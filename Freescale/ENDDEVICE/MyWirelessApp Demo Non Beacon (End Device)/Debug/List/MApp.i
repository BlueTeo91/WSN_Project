...
static void    App_TransmitUart2Data(uint8_t);
...
static tmrTimerID_t mTimer_c = 0xFF;
...
{ 
  ...
  Uart2_SetBaud(((UartBaudRate_t) 19200));
  Uart2_SetRxCallBack(UartRxCallBack);
   
  List_ClearAnchor(&mMlmeNwkInputQueue); 
  List_ClearAnchor(&mMcpsNwkInputQueue);
...
{ 
  ...
  if (events & (1 << 2))
  {
    ...
    pMsgIn = List_RemoveHead(&mMlmeNwkInputQueue);
    ...
    {               
      ...
      if(rc == errorNoError)
      {
        ...
        { MM_Free(((nwkMessage_t *)pMsgIn)->msgData . beaconNotifyInd . pBufferRoot); (((nwkMessage_t *)pMsgIn)->msgData . beaconNotifyInd . pBufferRoot) = (( void * )( 0 )); };
  ...
  case stateListen:
    ...
    {      
      ...
      if(1 == 1)
      {
      if(Uart2_GetByteFromRxBuffer((uint8_t*)&val))
      {
        valc=(unsigned char)(val -'0');
        UartUtil_Tx((unsigned char*)&val, sizeof(val));
        App_TransmitUart2Data(val);
      }
       
      }
      else if(Uart2_GetByteFromRxBuffer == 0)
      {
        UartUtil_Print("Not Enabled Uart2 .. buffer is empty\n\r", gAllowToBlock_d);
      }
      else
      {
        UartUtil_Print("Not Enabled Uart2 .. secondo controllo\n\r", gAllowToBlock_d);
      }
  ...
  {      
    ...
    pMsgIn = List_RemoveHead(&mMcpsNwkInputQueue);
    if (pMsgIn)
    {              
  ...
  if(((&mMcpsNwkInputQueue)->pHead != 0))
    TS_SendEvent(gAppTaskID_c, (1 << 3));
  if(((&mMlmeNwkInputQueue)->pHead != 0))
    TS_SendEvent(gAppTaskID_c, (1 << 2));  
}
...
static void UartRxCallBack(void) 
{
  uint8_t pressedKey;
    ...
    TS_SendEvent(gAppTaskID_c, (1 << 1));
  }else{
	  (void)Uart2_GetByteFromRxBuffer(&pressedKey);
  }
}
...
static uint8_t App_StartScan(uint8_t scanType)
{
  ...
  pMsg = MM_Alloc(sizeof(mlmeMessage_t));
  if(pMsg != (( void * )( 0 )))
  {
    ...
    pScanReq->scanChannels[0] = (uint8_t)((0x8000)     & 0xFF);
     
    pScanReq->scanChannels[1] = (uint8_t)((0x8000>>8)  & 0xFF);
     
    pScanReq->scanChannels[2] = (uint8_t)((0x8000>>16) & 0xFF);
     
    pScanReq->scanChannels[3] = (uint8_t)((0x8000>>24) & 0xFF);
    
 
    pScanReq->scanDuration = 3;
    
     
    if(((NWK_MLME_SapHandler)((void *)(pMsg))) == 0x00)
    {
      UartUtil_Print("Done\n\r", gAllowToBlock_d);
      return errorNoError;
    }
...
static uint8_t App_HandleScanActiveConfirm(nwkMessage_t *pMsg)
{
  ...
  {    
    ...
    while ((( void * )( 0 )) != pDescBlock)
    {
      ...
      { MM_Free(pBlock); (pBlock) = (( void * )( 0 )); };
    }
  }
  
  if (pDescBlock)
      { MM_Free(pDescBlock); (pDescBlock) = (( void * )( 0 )); };
...
static uint8_t App_HandleAssociateConfirm(nwkMessage_t *pMsg)
{
  ...
  if ( pMsg->msgData.associateCnf.status == 0x00) 
  {
...
static uint8_t App_HandleMlmeInput(nwkMessage_t *pMsg)
{
  ...
  switch(pMsg->msgType) {
  case gNwkPollCnf_c:
    if(pMsg->msgData.pollCnf.status != 0x00)
    {
...
static void App_HandleMcpsInput(mcpsToNwkMessage_t *pMsgIn)
{
  ...
  case gMcpsDataCnf_c:
    ...
    {
      mcPendingPackets--;
      UartUtil_Print("The data packet has been sent to network layer ...\n\r", gAllowToBlock_d);
    }
    break;

  case gMcpsDataInd_c:
    
    UartUtil_Print("Data indication has come for a data packet\n\r", gAllowToBlock_d);
...
static void App_TransmitUartData(void)
{   
  static uint8_t keysBuffer[32];
  static uint8_t keysReceived = 0;
  
   
  if( keysReceived < 32 ) 
  {
    if(Uart1_GetByteFromRxBuffer(&keysBuffer[keysReceived])) 
    {
    keysReceived++;
    }
  }
  ...
  if( (mcPendingPackets < 2) && (mpPacket == (( void * )( 0 ))) ) 
  {
    
 
    mpPacket = MM_Alloc(sizeof(nwkToMcpsMessage_t));
  }
  ...
  if(mpPacket != (( void * )( 0 )))
  {
      ...
      (void)((NWK_MCPS_SapHandler)((void *)(mpPacket)));
...
static void App_TransmitUart2Data(uint8_t value)
{   
  static uint8_t keysBuffer[32];
  static uint8_t keysReceived = 0;
  ...
  if(mpPacket != (( void * )( 0 )))
  {
      ...
      mpPacket->msgData.dataReq.pMsdu = &value;
      ...
      mpPacket->msgData.dataReq.dstAddrMode = mCoordInfo.coordAddrMode;
      mpPacket->msgData.dataReq.srcAddrMode = mAddrMode;
      mpPacket->msgData.dataReq.msduLength = sizeof(value);
...
{ 
  ...
  if(mWaitPollConfirm == 0)
  {
     
    mlmeMessage_t *pMlmeMsg = MM_Alloc(sizeof(mlmeMessage_t));
    if(pMlmeMsg)
    {
      ...
      pMlmeMsg->msgData.pollReq.securityEnable = 0;
      
       
      if(((NWK_MLME_SapHandler)((void *)(pMlmeMsg))) == 0x00)
      {
...
uint8_t MLME_NWK_SapHandler(nwkMessage_t * pMsg)
{
  ...
  List_AddTail((&mMlmeNwkInputQueue), (pMsg));
  TS_SendEvent(gAppTaskID_c, (1 << 2));
  return 0x00;
}
...
uint8_t MCPS_NWK_SapHandler(mcpsToNwkMessage_t *pMsg)
{
  ...
  List_AddTail((&mMcpsNwkInputQueue), (pMsg));
  TS_SendEvent(gAppTaskID_c, (1 << 3));
  return 0x00;
}

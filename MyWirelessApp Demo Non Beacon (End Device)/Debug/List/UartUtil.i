...
void UartUtil_Tx(uint8_t * pData, uint8_t length)
{
  ...
  while(!Uart2_Transmit((unsigned char const*)pData,(index_t)length,UartUtil_TxCallback));
   
  while (Uart2_IsTxActive());      
}
...
void UartUtil_Print(uint8_t* pString, uartUtilBlock_t allowToBlock)
{
  ...
  {
    (void)Uart2_Transmit((unsigned char const*)pString,(index_t)i,UartUtil_TxCallback);
  }
}
...
void UartUtil_PrintHex(uint8_t *hex, uint8_t len, uint8_t flags)
{
  ...
  while(len)
  {
    ...
    (void)Uart2_Transmit((unsigned char const*)hexString,(index_t)i,UartUtil_TxCallback);   
  }
...
void UartUtil_ConfigureStopMode(bool_t enterStopMode)
{
  ...
  {  
    ...
    UartOpenCloseTransceiver((1), 0x00000001UL, 0);            
  }  
  else
  {
      
    
    UartOpenCloseTransceiver((1), 0x00000002UL, 1); 

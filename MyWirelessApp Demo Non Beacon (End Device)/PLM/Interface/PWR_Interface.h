...
typedef  union {
  uint8_t AllBits;
  struct {
    unsigned int   FromHibernate   :1;                // Comming from Hibernate
    unsigned int   FromDoze        :1;                // Comming from Doze
    unsigned int   FromWait        :1;                // Comming from Wait
    unsigned int   FromReset       :1;                // Comming from Reset
    unsigned int   FromKBI         :1;                // Wakeup by KBI interrupt
    unsigned int   FromTimer       :1;                // Wakeup by Wakeup-timer

...
static index_t maSci1TxBufLenTable[3];
static index_t mSci1TxBufRefLeadingIndex;    
static index_t mSci1TxBufRefTrailingIndex;   

 
static SciTxBufRef_t maSci1TxCallbackTable[3];
...
static unsigned char maSci1RxBuf[32];
...
bool_t Uart1_GetByteFromRxBuffer(unsigned char *pDst) {
  ...
  return UartGetByteFromRxBuffer(0, pDst);
...
bool_t Uart1_IsTxActive(void) {
  return ((Uart1_TxInProgress) || (((UartRegs_t *)0x80005000)->Utxcon != 32));
}                                                   
...
void Uart_ModuleInit(void) {
  ...
  res = UartOpen(0,24000);  
  res |= UartSetConfig(0, &pConfig);  

  UartSetCTSThreshold(0, 8);

  
  res |= UartSetTransmitterThreshold(0, 5);
  res |= UartSetReceiverThreshold(0, 5);

  
  pCallback.pfUartWriteCallback = Uart1_TxIsr;
  pCallback.pfUartReadCallback = Uart1_RxIsr;

  res |= UartSetCallbackFunctions(0, &pCallback);
  ...
  UartReadData(0, maSci1RxBuf, sizeof(maSci1RxBuf), 1);       
...
void Uart1_SetRxCallBack(void (*pfCallBack)(void)) {
  ...
  UartOpenCloseTransceiver(0, 0x00000002UL, (( void * )( 0 )) != pfSci1RxCallBack);  
}       
...
bool_t Uart1_Transmit(unsigned char const *pBuf, index_t bufLen, void (*pfCallBack)(unsigned char const *pBuf)) {
  ...
  if(!Uart1_TxInProgress) 
  {
     ...
     UartWriteData(0, (uint8_t*) pBuf, bufLen);     
  } 
...
void Uart1_TxIsr(UartWriteCallbackArgs_t* args) {
  ...
  if(pfCallBack)
  {
      ...
      if (++maSci1TxCallbackLeadingIndex >= ((sizeof(maSci1TxBufRefTable) / (sizeof(maSci1TxBufRefTable[0])))))
      {
  ...
  if(maSci1TxBufLenTable[mSci1TxBufRefTrailingIndex] != 0)
  {
     ...
     UartWriteData(0, (uint8_t*)maSci1TxBufRefTable[mSci1TxBufRefTrailingIndex].pBuf, maSci1TxBufLenTable[mSci1TxBufRefTrailingIndex]);
  }
  else
  {
     Uart1_TxInProgress = 0;
  }         
}                                        
...
void Uart_Task(event_t events)
{
  ...
  if( events & (1<<2) ) {   
    ...
    while (maSci1TxCallbackLeadingIndex != maSci1TxCallbackTrailingIndex) {    
      ...
      if (++maSci1TxCallbackTrailingIndex >= ((sizeof(maSci1TxCallbackTable) / (sizeof(maSci1TxCallbackTable[0]))))) {
